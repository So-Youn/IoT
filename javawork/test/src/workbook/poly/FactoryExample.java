...
public class FactoryExample {
	...
	public void getResult(Factory mainFactory, char mainFactorySkill, IWorkingTogether partner) {
		...
		if (mainFactory instanceof CarFactory) {
			System.out.println("1. 하루 생산량 = " + ((CarFactory)mainFactory).makeProducts(mainFactorySkill));
			System.out.println("2. 파트너공장 "+((CarFactory)partner).getName()+"협력 시 추가 생산량 "+((CarFactory)partner).workTogether(partner));
		}
		if (mainFactory instanceof TVFactory) {
			System.out.println("1. 하루 생산량 = " + ((TVFactory)mainFactory).makeProducts(mainFactorySkill));
			System.out.println("2. 파트너공장 "+((TVFactory)partner).getName()+"협력 시 추가 생산량 "+((TVFactory)partner).workTogether(partner));
		}
